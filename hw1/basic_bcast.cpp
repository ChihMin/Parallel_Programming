...
*/
    ...
    if (rank < rank_num) {
        ...
        for (int i = 0; !is_completed || !(i % 2); ++i) {
            do_swap = false;
            //printf("rank %d : i = %d\n", rank, i);
            ...
            // if (do_swap) printf("rank %d swap %d\n", rank, i);
            MPI_Allreduce (&do_swap, &is_completed, 1,
                            MPI_INT, MPI_LOR, MPI_COMM_WORLD);
            is_completed = !is_completed; //inverse
            //printf("rank %d = is _ completed = %d\n", is_completed, i);
        }
    } else {
        do_swap = false;
